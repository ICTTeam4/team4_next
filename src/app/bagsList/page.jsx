...
function Page(props) {
  ...
  const API_URL = `${LOCAL_API_BASE_URL}/searchItems/bagsList`; // OuterList API 엔드포인트
  ...
  useEffect(() => {
    const fetchSearchResults = async () => {
      // 현재 카테고리가 "bagsList"인지 확인
      if (!searchKeyword || category !== "가방")
      ...
      try {
        const response = await axios.get(API_URL , {
          params: {
            ...
            category: category, // bagsList 고정 카테고리
          },
        });
