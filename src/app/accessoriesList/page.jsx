...
function Page(props) {
  ...
  const API_URL = `${LOCAL_API_BASE_URL}/searchItems/accessoriesList`; // OuterList API 엔드포인트
  ...
  useEffect(() => {
    const fetchSearchResults = async () => {
      // 현재 카테고리가 "accessoriesList"인지 확인
      if (!searchKeyword || category !== "패션잡화")
      ...
      try {
        const response = await axios.get(API_URL , {
          params: {
            ...
            category: category, // accessoriesList 고정 카테고리
          },
        });
